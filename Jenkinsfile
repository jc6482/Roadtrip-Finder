  ...
  stages {
    /*stage('SonarQube Analysis') {
      steps {
        withSonarQubeEnv('Eniworks') {
          container('sonarscan') {
              sh "sonar-scanner -Dsonar.projectKey=template:template-spring -Dsonar.sources=src"
          }
        }
      }
    }
    */
    stage('Build/Push Docker Image') {
      steps {
        container('kaniko') {
            sh "/kaniko/executor --context . --cache --destination=enigodupont/roadtripfinder:default_tag_change_me --destination=enigodupont/roadtripfinder:$BUILD_NUMBER"
        }
      }
    }
        ...
        withCredentials(bindings: [sshUserPrivateKey(credentialsId: '8fe21223-8d56-49ea-8eb5-6dc0171c102f', keyFileVariable: 'GIT_SSH_KEY', passphraseVariable: '', usernameVariable: '')]) {
          container('git') {
            ...
            sh '''
              ...
              git clone git@github.com:enigodupont/Roadtrip-Finder.git
              cd Roadtrip-Finder
